...
function Enemy(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.6, 0.6);
  this.move1 = Math.random() * Math.PI * 2;
}
...
Enemy.prototype.act = function(step) {
  this.move1 += step * move1Speed;
  var move1Pos = Math.sin(this.move1) * move1Dist;
  this.pos = this.basePos.plus(new Vector(0, move1Pos));
};
...
Player.prototype.moveY = function(step, level, keys) {
  ...
  var newPos = this.pos.plus(motion);
  var obstacle = level.obstacleAt(newPos, this.size);
  // The floor is also an obstacle -- only allowed player
  // to jump if they are touching some obstacle.
  if(obstacle) {
...
Level.prototype.playerTouched = function(type, actor) {
  if (type == 'coin') {
    this.actors = this.actors.filter(function(other) {
      ...
      return this.pos = new Vector(10, 10)
    }
}
